...
export function calculateCalibratedTransform(
  asset: Types.AssetLayer_Asset
): Types.AssetLayer_Asset_AssetTransform {
...
const AssetSizer: React.FunctionComponent<Props> = ({asset, onUpdate}) => {
  ...
  const [calibration, setCalibration] =
    useState<Types.AssetLayer_Asset_AssetCalibration>();
      ...
      {calibration && asset && (
        ...
        >
          ...
          <DialogContent sx={{width: VISUAL_ASSET_SIZER_SIZE}}>
            <Typography>
              Drag the corners of the selection box to align with the existing
              grid on your map. Then drag the box to match the size of the grid
              squares.
            </Typography>
            <br />
            <VisualAssetSizer
              asset={{...asset, calibration}}
