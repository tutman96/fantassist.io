...
const NetworkButton: React.FunctionComponent = () => {
  ...
  const dialog = (
    <Dialog open={showDialog} onClose={() => setShowDialog(false)}>
      <DialogTitle>Connect to Network Display</DialogTitle>
      <DialogContent>
        <DialogContentText>
        ...
        <TextField
          ...
          value={sessionCode}
          onChange={e =>
            setSessionCode(e.target.value.trim().toLocaleUpperCase())
          }
        />
      </DialogContent>
      <DialogActions>
  ...
  if (
    connectionState !== ChannelState.DISCONNECTED &&
    !(connection instanceof WebRTCApiChannel)
  ) {
    button = (
      <Button onClick={connect} startIcon={<LeakAdd />} fullWidth disabled>
    ...
    button = (
      <Button onClick={connect} startIcon={<LeakAdd />} fullWidth>
        Connect to Network Display (beta)
      </Button>
    );
  }
  return (
