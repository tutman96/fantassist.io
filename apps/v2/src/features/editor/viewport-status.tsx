...
export function EditorGestureHints({ effectTool, tool }: { readonly effectTool: EffectTool; readonly tool: EditorTool }) {
  ...
  return (
    <div className="pointer-events-none absolute bottom-3 left-4 hidden items-center gap-2.5 text-[10px] font-medium tracking-[0.08em] text-amber-50/55 uppercase md:flex">
      ...
      <span>{tool === "table" ? "Drag display to position" : polygon ? `Click to place ${tool === "effects" ? `effect ${editorEffectDefinition(effectTool).geometryNoun}` : "points"}` : tool === "light" ? "Click to place a light" : "Drag map to move"}</span>
      ...
      <span className="h-3 w-px bg-violet-300/15" />
      <span>{tool === "table" ? "Space + drag to roam" : polygon ? "Escape to cancel" : "Two-finger pan + zoom"}</span>
    </div>
  );
}
