...
With screen access, display selection is the first choice. Screen cards show only user-identifying details: browser label, primary/secondary and internal/external classification, physical resolution, and orientation. The launcher retains the remaining useful `ScreenDetailed` geometry internally for placement and generated identity. Selecting a card applies and persists its physical resolution before physical TV-size configuration. The selected generated ID is stored as `table_display_target`, restored after future screen discovery, and marked as the last-used display; Chromium does not expose a durable hardware identifier.
